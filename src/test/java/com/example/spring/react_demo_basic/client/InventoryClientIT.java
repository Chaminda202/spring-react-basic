...
@Testcontainers
public class InventoryClientIT {
    ...
    static void registerBaseUrl(DynamicPropertyRegistry registry) {
        ...
        registry.add("external.inventory-api.base-url", wiremock::getBaseUrl);
    }
    ...
    void startWireMock() {
        ...
        wireMockClient.register(
                WireMock.get(WireMock.urlPathEqualTo("/api/quantity"))
                        .withQueryParam("category", WireMock.equalTo("Electronic"))
                        .withQueryParam("name", WireMock.equalTo("Phone"))
                        .willReturn(WireMock.okJson("{\"category\":\"Electronic\",\"name\":\"Phone\",\"quantity\":1005}"))
        );
    }

    @Test
    void getProductInventoryTest() {
        StepVerifier.create(client.getProductInventory("Electronic", "Phone"))
